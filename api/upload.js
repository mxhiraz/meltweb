export const config={api:{bodyParser:{sizeLimit:"25mb"}}};
export default async function handler(req,res){
  if(req.headers["x-admin-pw"]!==(process.env.ADMIN_PW||"sex"))return res.status(401).json({error:"unauth"});
  if(req.method!=="POST")return res.status(405).end();
  const {path,contentBase64}=req.body||{};
  if(!path||!contentBase64)return res.status(400).json({error:"path+contentBase64 required"});
  const REPO=process.env.GH_REPO||"mxhiraz/meltweb";
  const TOKEN=process.env.GH_TOKEN;
  // check if file exists to get sha
  let sha;
  const probe=await fetch(`https://api.github.com/repos/${REPO}/contents/${path}`,{headers:{Authorization:`Bearer ${TOKEN}`,Accept:"application/vnd.github+json"}});
  if(probe.ok){const pd=await probe.json();sha=pd.sha}
  const r=await fetch(`https://api.github.com/repos/${REPO}/contents/${path}`,{method:"PUT",headers:{Authorization:`Bearer ${TOKEN}`,Accept:"application/vnd.github+json","Content-Type":"application/json"},body:JSON.stringify({message:`admin: upload ${path}`,content:contentBase64,sha,branch:"main"})});
  const d=await r.json();
  if(!r.ok)return res.status(500).json({error:d.message});
  res.json({ok:true,sha:d.content?.sha,commit:d.commit?.sha});
}
